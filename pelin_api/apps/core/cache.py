...
class CachedResourceMixin(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.method.upper() not in SAFE_METHODS:
            return super(CachedResourceMixin, self).dispatch(request, *args,
                                                             **kwargs)
        ...
        if (response_data is not None) and (key in keyset):
            ...
            with patch.object(self, handler_name, new=cached_handler):
                response = super(CachedResourceMixin, self).dispatch(request,
                                                                     *args,
                                                                     **kwargs)
        else:
            response = super(CachedResourceMixin, self).dispatch(request, *args,
                                                                 **kwargs)
